...
_int CRedStoneMonstrosityController::Update_Component(const _float& fTimeDelta)
{
	...
	_vec3 vPos = pRedStoneMonstrosity->Get_Component<CTransform>(L"Proto_TransformCom_root", ID_DYNAMIC)->m_vInfo[INFO_POS];
	_vec3 vTargetPos;
	...
	else
	{
		...
		vTargetPos = m_pTargetPlayer->Get_Component<CTransform>(L"Proto_TransformCom_root", ID_DYNAMIC)->m_vInfo[INFO_POS];
		_vec3 vDiff = vTargetPos - vPos;
		_float fDist = D3DXVec3Length(&vDiff);
